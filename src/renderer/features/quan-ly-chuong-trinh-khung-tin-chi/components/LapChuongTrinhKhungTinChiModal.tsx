...
import { validateAndCleanChiTietKhung } from '@renderer/features/lap-chuong-trinh-khung-tin-chi/utils';
...
const defaultValues = {
  ...
  idBacDaoTao: '',
  idChuyenNganh: '',
  chiTietKhungHocKy_TinChis: [],
};
interface Props {
  onClose: () => void;
  selectedData?: ChuongTrinhKhungTinChi | null;
}
export default function LapChuongTrinhKhungTinChiModal({ onClose, selectedData }: Props) {
  ...
  // Derived UI state
  const [hocKyRows, setHocKyRows] = useState<ChiTietKhungHocKy_TinChi[]>([]);
  const [monHocBatBuoc, setMonHocBatBuoc] = useState<MonHocBacDaoTaoForm[]>([]);
  const [monHocTuChon, setMonHocTuChon] = useState<MonHocBacDaoTaoForm[]>([]);
  ...
  useEffect(() => {
    if (isEdit && selectedData) {
      ...
      methods.setValue('chiTietKhungHocKy_TinChis', [...selectedData.chiTietKhungHocKy_TinChis]);
    }
  }, [selectedData]);
  ...
  const methods = useForm<ChuongTrinhKhungTinChi>({
    resolver: zodResolver(ChuongTrinhKhungTinChiSchema),
    defaultValues,
  });

  const { mutateAsync: insert } =
    useInsertMutation<LapChuongTrinhKhungTinChi>('ChuongTrinhKhungTinChi');
  const { mutateAsync: update } =
    useUpsertMutation<LapChuongTrinhKhungTinChi>('ChuongTrinhKhungTinChi');

  // useWatch for live form data
  const chiTietKhungHocKyTinChis =
    useWatch({
      control: methods.control,
      name: 'chiTietKhungHocKy_TinChis',
    }) ?? [];

  useEffect(() => {
    const hocKyTcMap = new Map<number, { batBuoc: number; tuChon: number }>();
    const chiTietKhungHocKyTinChis = methods.getValues(
      'chiTietKhungHocKy_TinChis',
    ) as ChiTietKhungHocKy_TinChi[];
    chiTietKhungHocKyTinChis.forEach((item) => {
      if (item.idMonHocBacDaoTao && item.hocKy) {
        ...
        if (item.isBatBuoc) entry.batBuoc += soTinChi;
        else entry.tuChon += soTinChi;
      }
    });

    const hocKys = Array.from(
      new Map(
        chiTietKhungHocKyTinChis.map((item) => [
          item.hocKy,
          {
            ...item,
            id: item.id ?? uuidv4(),
    ...
    const monHocBatBuoc = monHocRows
      .filter((x) => x.isBatBuoc)
      .map((x) => x.monHocBacDaoTao!)
      .filter(Boolean);
    setMonHocBatBuoc(monHocBatBuoc);
    const monHocTuChon = monHocRows
      .filter((x) => !x.isBatBuoc)
      .map((x) => x.monHocBacDaoTao!)
      .filter(Boolean);
    setMonHocTuChon(monHocTuChon);
  }, [chiTietKhungHocKyTinChis, hocKySelected]);

  const handleAddHocKy = methods.handleSubmit(
    () => {
      const current = methods.getValues('chiTietKhungHocKy_TinChis');
      const nextHocKy = Math.max(...current.map((c) => c.hocKy ?? 0), 0) + 1;

      methods.setValue('chiTietKhungHocKy_TinChis', [
        ...current,
        {
          id: uuidv4(),
          hocKy: nextHocKy,
  ...
  const handleRemoveHocKy = () => {
    ...
    methods.setValue('chiTietKhungHocKy_TinChis', newItems);
  };
  ...
  const handleAddMonHocBacDaoTao = (
    ...
    const current = methods.getValues('chiTietKhungHocKy_TinChis');
    methods.setValue('chiTietKhungHocKy_TinChis', [...current, ...newItems]);
  ...
  const handleSubmitAll = methods.handleSubmit(
    async (values) => {
      const result = validateAndCleanChiTietKhung(values.chiTietKhungHocKy_TinChis);
      ...
      const payload = {
        ...values,
        chiTietKhungHocKy_TinChis: result.data,
      };

...
import { PhieuMuonTraFilterState } from './validation';

const filterFields = [
  { key: 'maPhieu', label: 'Mã phiếu' },
  { key: 'tenNguoiMuon', label: 'Người mượn' },
  { key: 'maThietBi', label: 'Mã thiết bị' },
  { key: 'tenThietBi', label: 'Tên thiết bị' },
  { key: 'trangThaiText', label: 'Trạng thái' },
  { key: 'loaiDoiTuongText', label: 'Loại đối tượng' },
];

interface Props {
  onApply: (filters: PhieuMuonTraFilterState) => void;
  onReset: () => void;
}

export const PhieuMuonTraFilter = ({ onApply, onReset }: Props) => {
    ...
    <Stack spacing={1} sx={{ mb: 1 }}>
      <Autocomplete
        ...
        renderInput={(params) => (
          <TextField
            {...params}
            placeholder="Tìm theo Mã phiếu, Người mượn, Thiết bị, Trạng thái..."
            size="small"
            fullWidth
            InputProps={{
              ...params.InputProps,
